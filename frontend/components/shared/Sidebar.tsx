...
import { LayoutDashboard, Video, UserCheck, BarChart3, FileText, Users, Settings, Zap, LogOut, ChevronLeft, ChevronRight, Radio } from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 h-16 border-b border-white/[0.06] flex-shrink-0">
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#10b981] border-2 border-[#0a0a0f]" />
        </div>
        <AnimatePresence>
          {!collapsed && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
              <h1 className="text-base font-bold gradient-text leading-tight">ClassPulse</h1>
              <p className="text-[9px] text-[#64748b] tracking-[0.15em] uppercase">AI Monitor</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <AnimatePresence>
        {!collapsed && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mx-3 mt-3 mb-1 px-3 py-2 rounded-lg bg-[#10b981]/8 border border-[#10b981]/15">
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className={cn(
                ...
                active ? "bg-[#6366f1]/12 text-white" : "text-[#64748b] hover:text-white hover:bg-white/[0.03]"
              )}>
                {active && (
                ...
                <item.icon className={cn("w-[18px] h-[18px] flex-shrink-0", active && "text-[#818cf8]")} />
                <AnimatePresence>
                  {!collapsed && (
      ...
      <div className="px-3 py-3 border-t border-white/[0.06] space-y-2">
        ...
        <div className="flex gap-1">
          <button onClick={handleLogout} className={cn("flex items-center gap-2 py-2 rounded-lg text-[#64748b] hover:text-[#ef4444] hover:bg-[#ef4444]/8 transition", collapsed ? "justify-center w-full px-2" : "px-3 flex-1")}>
            <LogOut className="w-4 h-4" />
            {!collapsed && <span className="text-xs">Sign Out</span>}
          </button>
          <button onClick={() => setCollapsed(!collapsed)} className="py-2 px-2 rounded-lg text-[#64748b] hover:text-white hover:bg-white/[0.04] transition">
